...
from network_security_workspace.entity.artifact_entity import DataIngestionArtifact
...
class DataIngestion:
    ...
    def split_data_into_train_test(self, dataframe : pd.DataFrame):
        try:
            train_set, test_set = train_test_split(dataframe, train_size= self.data_ingestion_config.train_test_split_ratio, random_state=42)
            logging.info("Successfully split the data into train and test")
            
            dir_path = os.path.dirname(self.data_ingestion_config.training_file_path)
            os.makedirs(dir_path, exist_ok=True)

            logging.info("Exporting the training and testing file path")
            
            train_set.to_csv(self.data_ingestion_config.training_file_path, index=False, header=True)
            test_set.to_csv(self.data_ingestion_config.testing_file_path, index=False, header=True)
            
            logging.info("Successfully exported the training and testing file path")
